...
class AccountVoucherWizardSale(models.TransientModel):
        ...
        if self.env.context.get("active_id", False):
            if (
                ...
                raise exceptions.ValidationError(
                    _("Amount of advance is greater than residual amount on sale order")
                )
    ...
    def default_get(self, fields_list):
        ...
        if not sale_order_ids:
            return res
        sale_order_id = fields.first(sale_order_ids)
        sale_order = self.env["sale.order"].browse(sale_order_id)
        if "amount_total" in fields_list:
            res.update(
                {
                    "order_id": sale_order.id,
                    "amount_total": sale_order.amount_residual,
        ...
        if sale_order.company_id.currency_id != sale_order.currency_id:
    ...
    def _prepare_payment_vals(self, sale_order):
        ...
        return {
            ...
            "memo": self.payment_ref or sale_order.name,
    ...
    def make_advance_payment(self):
        ...
        if sale_order_ids:
            sale_order_id = fields.first(sale_order_ids)
            sale_order = purchase_obj.browse(sale_order_id)
            payment_vals = self._prepare_payment_vals(sale_order)
            payment = payment_obj.create(payment_vals)
            if bool(
                    self.env["ir.config_parameter"]
                            .sudo()
                            .get_param("sale_advance_payment.auto_post_advance_payments")
            ):
                payment.action_post()
